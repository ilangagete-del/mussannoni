...
from .base import banner_html, document_html, esc
...
def _division_summary_html(slip: SchoolResultSlip) -> str:
    ...
    rows: list[str] = []
    for drow in slip.division_summary:
        cells = f"<td>{esc(drow.sex)}</td>" + "".join(
            f"<td>{esc(drow.divisions.get(lbl, ''))}</td>" for lbl in labels
        )
...
def _students_html(slip: SchoolResultSlip) -> str:
    ...
    for st in slip.students:
        rows.append(
            "<tr>"
            f"<td>{esc(st.cno)}</td>"
            f'<td class="text">{esc(st.name)}</td>'
            f"<td>{esc(st.sex)}</td>"
            f"<td>{esc(st.aggregate)}</td>"
            f"<td>{esc(st.division)}</td>"
            f"<td>{esc(st.position)}</td>"
            f'<td class="text">{esc(st.detailed_subjects)}</td>'
            "</tr>"
        )
    return (
        '<table class="tmpl">'
